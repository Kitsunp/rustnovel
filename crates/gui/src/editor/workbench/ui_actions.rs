...
impl EditorWorkbench {
    pub(super) fn handle_asset_browser_actions(
        &mut self,
    ...
    pub(super) fn handle_composer_actions(
        &mut self,
    ...
    pub(super) fn handle_inspector_actions(
        &mut self,
        actions: Vec<crate::editor::InspectorAction>,
    ) {
        for action in actions {
            match action {
    ...
    fn handle_background_fit_changed(
        ...
        self.set_composer_background_fit_for_node(node_id, fit);
        self.node_graph.mark_modified();
        self.queue_editor_operation_with_values(
            "background_fit_changed",
            format!("Set background fit to {}", fit.label()),
            node_id.map(|id| format!("composer.background_fit[{id}]")),
            None,
            Some(fit.label().to_string()),
        );
    }
}
